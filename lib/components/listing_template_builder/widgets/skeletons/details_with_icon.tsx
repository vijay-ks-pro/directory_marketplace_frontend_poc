...
const DetailsWithIconWidgetSkeleton = ({ headerElement, borderRadius = '8px', customWidgetName } : WidgetSkeletonProps) => {
    return (
        <Flex w = '100%' direction={'column'} borderRadius={borderRadius} border = '1px' borderColor={'gray.200'}>
            <Flex borderTopRadius={borderRadius} w = '100%' h = '50px' bg = 'white' alignItems={'center'} justifyContent={'space-between'} pl = '15px'>
                <Heading size = 'sm'>{customWidgetName ?? `Details with Icon`}</Heading>
                {headerElement}
            </Flex>
...
export default DetailsWithIconWidgetSkeleton;